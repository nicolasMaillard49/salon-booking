...
export class MailService {
  ...
  private fromEmail = 'Nm.D.Barber <noreply@restaurants-bordeaux.com>';
  ...
  async sendCreatedToClient(appointment: any) {
    try {
      ...
      this.logger.warn(`Email client non envoyé : ${e.message}`);
    }
  }
  ...
  async sendCreatedToAdmin(appointment: any) {
    try {
      ...
      this.logger.warn(`Email admin non envoyé : ${e.message}`);
    }
  }
  ...
  async sendStatusUpdate(appointment: any) {
    try {
      ...
      this.logger.warn(`Email statut non envoyé : ${e.message}`);
    }
  }
}
